...
class BevBuddy extends Simulation {
  ...
  def createRpc(msg: String): String = {
  ...
  .exec(initSyncAndClientIds)
    ...
    .exec(http("Navigate to Categories")
      .post(uidlUrl)
      .headers(headers_69)
      .body(StringBody(createRpc("""{"type":"navigation","location":"categories","link":1}"""))).asJson
    ...
    .exec(http("Navigate to Reviews")
      .post(uidlUrl)
      .headers(headers_69)
      .body(StringBody(createRpc("""{"type":"navigation","location":"","link":1}"""))).asJson
      .check(regex("""Reviews"""))
      .check(syncIdExtract).check(clientIdExtract)
    )

  val scn = scenario("BevBuddy").repeat(1) {
    exec(chain_0)
  }
